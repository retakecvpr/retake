...
from ...generation import GenerationMixin, GenerationConfig, LogitsProcessorList, StoppingCriteriaList
...
class LongVideoGenerationMixin(GenerationMixin):
    def prepare_longvideo_prefill(self, kwargs, config):
        # Get model input
    ...
    def prepare_longvideo_decode(self, kwargs, past_key_values, rope_deltas, prefill_input_len):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Calculate the number of chunked prefill tokens
        chunk_frames = getattr(prefill_inputs["past_key_values"], "chunk_frames", None)
        if chunk_frames is None:
            chunk_prefill_size = None
        else:
            T, H, W = prefill_inputs['video_grid_thw'][0]
            factor = self.config.vision_config.spatial_merge_size**2 * self.config.vision_config.temporal_patch_size
        ...
        generated_tokens = super().generate(
            inputs=inputs,
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, LongVideoGenerationMixin):
        ...
        if image_grid_thw is not None or video_grid_thw is not None:
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                try:
                    ...
                    logger.warning("Shape of `position_ids` and `llm_positions` does not match. Probably due to cut off.")
                    logger.warning("position_ids.shape: " + str(position_ids.shape))
                    logger.warning("llm_positions.shape: " + str(llm_positions.shape))
                    input_len = position_ids.shape[-1]
                    llm_positions = llm_positions[:,:input_len]
                    position_ids[..., i, attention_mask[i] == 1] = llm_positions.to(position_ids.device)
    ...
    def compress_video_tokens(
        ...
        if visual_compression:
            ...
            # print("before compression compressed_memory_bank.shape", compressed_memory_bank.shape)
            ...
            # print("after compression compressed_memory_bank.shape", compressed_memory_bank.shape)
            video_embeds = compressed_memory_bank.flatten(1, 2)

            # Reforge the input
            # print("before compression input_ids.shape", input_ids.shape)
            input_ids = torch.cat([
                input_ids[:, :s_index],
                ...
                dim=1)
            # print("after compression input_ids.shape", input_ids.shape)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if grid_t < 128:
                    ...
                    video_embeds = []
                    for i in range(0, grid_t, 128):
                        pixel_values_videos_chunk = pixel_values_videos[i:i+128]
        ...
        if chunk_size is None:
            ...
            kvcache_compression = past_key_values.kvcache_compression
            ...
            for idx in range(num_chunks):
                ...
                grid_t, grid_h, grid_w = video_grid_thw[0]
                # Number of evicted frames
                num_frame_diff = (s - past_key_values.get_seq_length()) // (
                    grid_h * grid_w // self.config.vision_config.spatial_merge_size**2
                )
                position_ids_chunk[0] -= num_frame_diff
                outputs = self.model(
                    input_ids=None,
                    position_ids=position_ids_chunk,
                    attention_mask=attention_mask_chunk,
