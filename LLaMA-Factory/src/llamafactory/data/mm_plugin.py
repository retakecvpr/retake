...
class BasePlugin:
    ...
    def _get_video_sample_extracted_frames(self, frame_files: List[str], **kwargs) -> int:
    ...
    def _regularize_videos(self, videos: Sequence["VideoInput"], **kwargs) -> List[List["ImageObject"]]:
        ...
        dummy mode: load only the first frame.
        """
        results = []
        for video in videos:
            ...
            if isinstance(video, list):
                # Assume all frames are loaded into PIL Image and resized
                frames = video
                ...
                if os.path.isdir(video):
                    # `video` is a directory of sampled frames
                    frame_files = [
                        os.path.join(video, file) for file in list(sorted(os.listdir(video)))
                    ]
                    total_frames = len(frame_files)
                    sample_frames = self._get_video_sample_extracted_frames(frame_files, **kwargs)
                    sample_indices = np.linspace(0, total_frames - 1, sample_frames).astype(np.int32)
                    num_frames = sample_indices.shape[0]
                    if dummy:
                    ...
                    for frame_idx, frame in enumerate(container.decode(video_stream)):
                        if frame_idx in sample_indices:
                            ...
                            if len(frames) == sample_indices.shape[0]:
                                break
    ...
    def _get_mm_inputs(
        ...
        dummy mode: To avoid timeout in dataset.map() during pre-process, we only load 
        one frame for video and expand its `video_grid_thw` to num_frames, because
        data pre-process only requires the total number of visual tokens to be correct.
        ...
        if len(videos) != 0:
            videos, num_frames = self._regularize_videos(
                videos,
                image_resolution=getattr(processor, "video_resolution", 128),
                video_fps=getattr(processor, "video_fps", 1.0),
                video_maxlen=getattr(processor, "video_maxlen", 64),
                video_frame_extraction_fps=getattr(processor, "video_frame_extraction_fps", 5),
                dummy=dummy
            )
        ...
        if dummy:
            mm_inputs['video_grid_thw'][0][0] = num_frames // video_processor.temporal_patch_size
...
class Qwen2vlPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        mm_inputs = self._get_mm_inputs(images, videos, processor, dummy=True)
